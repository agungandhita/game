                    ...
                    <div class="relative group">
                        <select name="level_id" id="level_id" class="w-full pl-4 pr-10 py-3.5 rounded-2xl bg-gray-50 border-transparent focus:border-indigo-500 focus:bg-white focus:ring-0 appearance-none transition-all text-sm font-medium" required>
                            ...
                            @foreach($levels as $level)
                                <option value="{{ $level->id }}" {{ (old('level_id') ?? $question->level_id) == $level->id ? 'selected' : '' }}>{{ $level->title }} ({{ $level->world->name }})</option>
                            @endforeach
                        </select>
                    ...
                    <div class="relative group bg-white p-4 rounded-2xl border border-indigo-100 shadow-sm transition-all focus-within:ring-2 focus-within:ring-indigo-400">
                        <div class="flex items-center gap-4">
                            <input type="radio" name="correct_option" value="{{ $i }}" 
                                ...
                                class="w-6 h-6 text-indigo-600 bg-gray-50 border-gray-200 focus:ring-indigo-500 cursor-pointer transition-transform hover:scale-110">
                            <input type="text" name="options[{{ $i }}][content]" placeholder="Tulis jawaban ke-{{ $i + 1 }}..." 
                                value="{{ old('options.'.$i.'.content') ?? ($options[$i]->content ?? '') }}"
                                class="flex-1 bg-transparent border-none focus:ring-0 text-gray-700 font-medium placeholder-gray-300">
                        </div>
                    </div>
                    @endfor
                </div>
